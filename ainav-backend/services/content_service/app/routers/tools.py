...
from ..dependencies import get_db, get_current_user_id, get_optional_user
...
async def list_tools(
    ...
    """
    List all tools with optional personalization.
    When user is authenticated, can show bookmarks/favorites in the future.
    """
...
async def get_tool(
    ...
    When user is authenticated, can show bookmark/favorite status in the future.
    """
    repo = ToolRepository(db)
...
async def create_tool(
    ...
    Only authenticated users can add tools to the catalog.
    """
    repo = ToolRepository(db)
...
async def update_tool(
    ...
    """
    Update a tool and its scenario associations (requires authentication).
    Only authenticated users can modify tools in the catalog.
    """
    repo = ToolRepository(db)
...
async def delete_tool(
    ...
    Only authenticated users can remove tools from the catalog.
    """
    repo = ToolRepository(db)
