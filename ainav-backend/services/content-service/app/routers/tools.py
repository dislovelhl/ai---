...
async def update_tool(tool_id: UUID4, tool_in: ToolUpdate, db: AsyncSession = Depends(get_db)):
    """Update a tool and its scenario associations."""
    repo = ToolRepository(db)
...
async def delete_tool(tool_id: UUID4, db: AsyncSession = Depends(get_db)):
    """Delete a tool."""
    repo = ToolRepository(db)
